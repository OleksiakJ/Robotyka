...
int main(void) {
    ...
    if (wyznacz_punkt(siatka, start.w_x, start.w_y).wart == 5 || wyznacz_punkt(siatka, cel.w_x, cel.w_y).wart == 5) {
        cout << "Start lub koniec znajduje sie na blokadzie";
        return 0;
    }
    if (!czyNalezy(start.w_x, start.w_y, siatka) || !czyNalezy(cel.w_x, cel.w_y, siatka)) {
        cout << " Punkt startu lub koñca nie lezy na mapie ";
        return 0;
    }
    ...
    while (!lista_otwarta.empty()) {
        if (poz.w_x == czy_petla[0] && poz.w_y == czy_petla[1]) {
            cout << "Brak sciezki";
            return 0;
        }
        ...
        poz_d.rodzic_x = parent_x;
        poz_d.rodzic_y = parent_y;
        ...
        poz_l.rodzic_x = parent_x;
        poz_l.rodzic_y = parent_y;
